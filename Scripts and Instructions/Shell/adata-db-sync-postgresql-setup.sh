...
function check_psql_superuser {
	set +e
	psql -h /media/artur/ADATA\ Exter/Projects/postgres -p 5433 -l "${PGDATABASE}" > /dev/null 2>&1 || psql -l > /dev/null 2>&1
	if test $? -ne 0 ; then
		echo
...
function check_db_exists {
	set +e
	count=$(psql -h /media/artur/ADATA\ Exter/Projects/postgres -p 5433 -l "${PGDATABASE}" | grep -c "${PGDATABASE} ")
	...
	count=$(psql -h /media/artur/ADATA\ Exter/Projects/postgres -p 5433 -l "${PGDATABASE}" | grep "${PGDATABASE} " | cut -d \| -f 3 | grep -c UTF8)
...
function create_db {
	#if test "$( psqla "${PGDATABASE}" -tAc "SELECT 1 FROM pg_database WHERE datname='${PGDATABASE}'" )" != '1' ; then
	#	createdb -T template0 --owner="${PGUSER:-$(whoami)}" --encoding=UTF8 "${PGDATABASE}"
	#fi
	createdb -U artur -h /media/artur/ADATA\ Exter/Projects/postgres -p 5433 --template template0 --owner="${PGUSER:-$(whoami)}" --encoding=UTF8 "${PGDATABASE}"
}



function drop_db {
	if test "$( psql -h /media/artur/ADATA\ Exter/Projects/postgres -p 5433 "${PGDATABASE}" -tAc "SELECT 1 FROM pg_database WHERE datname='${PGDATABASE}'" )" = '1' ; then
		psql -h /media/artur/ADATA\ Exter/Projects/postgres -p 5433 "${PGDATABASE}" --command="DROP OWNED BY CURRENT_USER cascade;"
	fi
}
...
function create_snapshot {
	...
	pg_dump -h /media/artur/ADATA\ Exter/Projects/postgres -p 5433 --no-owner --schema=public "${PGDATABASE}" > "${tmp_dir}/$1.sql"
...
function restore_snapshot {
	...
	mv "${lstate_file}" "$2"
	psql -h /media/artur/ADATA\ Exter/Projects/postgres -p 5433 --dbname="${PGDATABASE}" -f "${db_file}"
