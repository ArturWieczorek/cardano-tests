...
user=$(echo $USER)
environments=('shelley-qa' 'sanchonet' 'preview' 'preprod' 'mainnet')

config_dir="cardano-configs"
db_sync_config_dir="db-sync-configs"
adata_db_sync_config_dir="adata-db-sync-configs"
adata_port=5433
...
get_cardano_configuration_files()
{
    ...
    mkdir "${adata_db_sync_config_dir}"

    echo "Downloading all config files. This may take few seconds..."
    for env in ${environments[@]}; do
        mkdir "${env}"
        cd "${env}"
        wget --quiet "https://book.play.dev.cardano.org/environments/${env}/config.json"
        wget --quiet "https://book.play.dev.cardano.org/environments/${env}/db-sync-config.json"
        wget --quiet "https://book.play.dev.cardano.org/environments/${env}/submit-api-config.json"
        wget --quiet "https://book.play.dev.cardano.org/environments/${env}/topology.json"
        wget --quiet "https://book.play.dev.cardano.org/environments/${env}/byron-genesis.json"
        wget --quiet "https://book.play.dev.cardano.org/environments/${env}/shelley-genesis.json"
        wget --quiet "https://book.play.dev.cardano.org/environments/${env}/alonzo-genesis.json"
        wget --quiet "https://book.play.dev.cardano.org/environments/${env}/conway-genesis.json"
        cd ..
