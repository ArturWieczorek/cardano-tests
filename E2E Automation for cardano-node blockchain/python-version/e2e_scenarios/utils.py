...
def get_stake_address_info(stake_addr):
    try:
        ...
        print(f"stake_address_info: {output_json}")
...
def calculate_tx_fee(src_addr, dst_addr_list, tx_ttl, witness_count=0, byron_witness_count=0, **options):
    ...
    # dst_addr could be a single address or a list of addresses
    get_protocol_params()
    # create a dummy dst_amounts_list in order to build the transaction
    if isinstance(dst_addr_list, str):
...
def build_raw_transaction(ttl, fee, src_address, dst_addr_list, dst_amounts, **options):
    ...
    # dst_addr could be 1 single address or a list of addresses
    print(f"Building the raw transaction...")
    out_file = "tx_raw.body"

    if isinstance(dst_addr_list, str):
        dst_addr_list = [dst_addr_list]
    if isinstance(dst_amounts, (int, float, str)):
        dst_amounts = [dst_amounts]
...
def send_funds(src_address, tx_fee, tx_ttl, **options):
    ...
    print(f"--------------------------------------------------------")
    print(f"src_address         : {src_address}")
    print(f"destinations_list   : {destinations_list}")
    print(f"transferred_amounts : {transferred_amounts}")
    print(f"signing_keys        : {signing_keys_list}")
    print(f"tx_fee              : {tx_fee}")
    print(f"tx_ttl              : {tx_ttl}")
    print(f"certificates        : {certificates_list}")
...
def gen_pool_registration_cert(pool_pledge, pool_cost, pool_margin, node_vrf_vkey_file, node_cold_vkey_file,
                               stake_addr_vkey_file, location, node_name, **options):
    # if there are multiple owners:
    #   - stake_addr_vkey_file = a list of files (stake address vkey of owners)
    #   - the rewards will be sent/configured to the first stake address from the above list
    # NOTE: each pool should have only 1 reward address
    ...
    if isinstance(stake_addr_vkey_file, list):
        owners_cmd = ''.join([" --pool-owner-stake-verification-key-file " +
                              stake_addr_file for stake_addr_file in stake_addr_vkey_file])
        reward_cmd = " --pool-reward-account-verification-key-file " + stake_addr_vkey_file[0]
        cmd = cmd + owners_cmd + reward_cmd
    elif isinstance(stake_addr_vkey_file, str):
        owners_cmd = " --pool-owner-stake-verification-key-file " + stake_addr_vkey_file
        reward_cmd = " --pool-reward-account-verification-key-file " + stake_addr_vkey_file
        cmd = cmd + owners_cmd + reward_cmd
    try:
