...
./register-stake-pool.sh -n shelley_qa
...
get_random_number () {
    local low=${1:-100}
    local up=${2:-1000}
    local random_number=$(shuf -i ${low}-${up} -n 1)
    echo $random_number
}
...
MIN_POOL_COST=$(get_network_param "minPoolCost")
...
random_number_2=$(get_random_number 10 99)

pool_metadata_line1="\"name\": \"QA Pool "$random_number_1\"","
pool_metadata_line2='"description": "Test DB-Sync",'
pool_metadata_line3="\"ticker\": \"QA_"$random_number_2\"","
pool_metadata_line4='"homepage": "http://artur_pool.pl/"'

# Create a JSON file with your pool’s metadata
#   {
#       "name": "Artur_Pool_1",
#       "description": "Test DB-Sync",
#       "ticker": "QA_123",
#       "homepage": "http://artur_pool.pl/"
#   }


pool_metadata_file=pool_metadata.json

rm $pool_metadata_file

echo "{" >> $pool_metadata_file
echo $pool_metadata_line1 >> $pool_metadata_file
echo $pool_metadata_line2 >> $pool_metadata_file
echo $pool_metadata_line3 >> $pool_metadata_file
echo $pool_metadata_line4 >> $pool_metadata_file
echo "}" >> $pool_metadata_file


# Get the hash of your metadata JSON file:

pool_metadata_file_hash=$(cardano-cli stake-pool metadata-hash --pool-metadata-file $pool_metadata_file)
pool_metadata_url="https://bit.ly/3d2738r"
...
--metadata-hash $pool_metadata_file_hash \
...
stake_addr=$(cat ../keys/stake.addr)

extended_payment_addr=$(cat ../keys/extended-payment.addr)

extended_stake_addr=$(cat ../keys/stake2.addr)
...
change=$(( $tx_balance - $POOL_DEPOSIT - $fee))
...
--fee $fee \
--out-file txs/pool_tx_${tx_number}.raw \
...
--tx-body-file txs/pool_tx_${tx_number}.raw \
--signing-key-file ../keys/payment.skey \
--signing-key-file ../keys/stake2.skey \
--signing-key-file cold.skey \
--testnet-magic $NETWORK_MAGIC \
--out-file txs/pool_tx_${tx_number}.signed

# Submit the transaction:

cardano-cli transaction submit \
--tx-file txs/pool_tx_${tx_number}.signed \
--testnet-magic $NETWORK_MAGIC \

tx_hash=$(cardano-cli transaction txid --tx-body-file $txs_dir/pool_tx_${tx_number}.raw)
...
#check delegation
cardano-cli query stake-address-info --testnet-magic 3 --address $(cat cardano-node/keys/stake2.addr) --mary-era | jq .[0].'delegation'
